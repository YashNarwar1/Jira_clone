...
.patch(
  ...
  .post(
    ...
    async (c) => {
     ...
     // if there is a task in the list, we will add 100 to the position of the last task
       const newPostion = highestPositionTask.documents.length > 0
  ...
  .get(
    ...
    async (c) => {
     ...
     if(!currentMember){
     return c.json({erro: "Unauthorized"}, 401) 
    };
