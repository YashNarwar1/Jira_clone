...
export const DataKanban = ({ data, onChange }: DataKanbanProps) => {
  const [tasks, setTasks] = useState<TaskState>(() => {
    ...
    // adding tasks to the board on there respectve postions
    data.forEach((task) => {
      initialTasks[task.status].push(task);
    });
    // sorting the tasks according to their postions
    Object.keys(initialTasks).forEach((status) => {
      initialTasks[status as TaskStatus].sort(
        (a, b) => a.position - b.position
      );
    });
    // then returning the tasks value
    return initialTasks;
  });
  ...
  const onDragEnd = useCallback(
    (result: DropResult) => {
      ...
      setTasks((prevTasks) => {
        ...
        //if there's no moved task return the prev state
        if (!movedTask) {
